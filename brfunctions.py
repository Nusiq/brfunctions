...
from pymclevel import ( TAG_List, TAG_Byte, TAG_Int, TAG_Compound, TAG_Short,
    TAG_Double, TAG_Float, TAG_String, TAG_Long, TAG_Int_Array, alphaMaterials,
    )
import json
import string
import os
...
class Builder:
    ...
    def place_comparator(x,y,z, level, direction, signal, compare):
        ...
        comparator_directions = {Builder.z_up:0, Builder.x_down:1,
    ...
    def place_cmdblock(x, y, z, level, cmdBlockType, direction, command,
        ...
        tileCB["LastOutputParams"] = TAG_List([TAG_String(u'1'),
        ...
        Builder.place_block(x, y, z,
            alphaMaterials[cb_types[cmdBlockType],direction], level
            )
    place_cmdblock = staticmethod(place_cmdblock)

    
    def place_hooper(x, y, z, level, items,direction, disabled):
        hopper_directions = {Builder.y_down:0, Builder.z_down:2,
            Builder.z_up:3, Builder.x_down:4, Builder.x_up:5,
            }
...
class Token:
    ...
    def __str__(self):
        ...
        return str({'type':types[self.type],'value':self.value,
            'name':self.name,
            })
...
class Parser:
    state_blocks = ('wool 0', 'wool 1', 'wool 2', 'wool 3', 'wool 4', 'wool 5',
        'wool 6','wool 7', 'wool 8', 'wool 9', 'wool 10', 'wool 11', 'wool 12',
        'wool 13', 'wool 14', 'wool 15', 'stained_hardened_clay 0',
            ...
            for c in command:
                if state==start:
                    ...
                    if c == '-':
                        i+=1
                        state=sign
                    elif c == '.':#If type is not global decimals are allowed
                        i+=1
                        state=fraction
                    elif c in string.digits:
                        i+=1
            ...
            if state == start:
                ...
                if i < input_len:    
                    if input[i] == '`':
                        ...
                        if i >= input_len:
                            tokens.append(Token(Token.command, '', None,
                                input_line_index, input_file_name))
                            state = end
                        else:
                            consumed_input = i
                ...
                i += length
                if i >= input_len:
                    tokens.append(Token(Token.command, input[consumed_input:i],
                        None, input_line_index, input_file_name))
                    return tokens
                if input[i] == '`':
                    tokens.append(Token(Token.command, input[consumed_input:i],
                        None, input_line_index, input_file_name))
                ...
                if input[i] == '/':
                    ...
                    if input[i] == 's':#state
                        ...
                        if i >= input_len:
                            raise Exception('Unexpected end of line '
                                +str(input_line_index)+' in file '
                                +input_file_name+' Code:c08d9d')
                        if input[i] == '[': i+=1
                        if i >= input_len:
                            raise Exception('Unexpected end of line '
                                +str(input_line_index)+' in file '
                                +input_file_name+' Code:6bd8f2')
                        length, _ = Parser.cut_word(input[i:],separators=']',
                            escape_characters='\\',allowed_chars=None)
                        name = input[i:length+i]
                        i += length+1
                        if i >= input_len:
                            raise Exception('Unexpected end of line '
                                +str(input_line_index)+' in file '
                                +input_file_name+' Code:gc4o14')
                        if input[i] == '(':
                            i+=1
                            if i >= input_len:
                                raise Exception('Unexpected end of line '
                                    +str(input_line_index)+' in file '
                                    +input_file_name+' Code:0s8x5o')
                            (length, position,
                                success, error,
                                ) = Parser.parse_coordinates(input[i:],
                                    count=3,allow_fraction=False)
                            if not success:
                                raise Exception(error+' at line '
                                    +str(input_line_index)+' in file '
                                    +input_file_name+' Code:74gorv')
                            i += length
                            if not success:
                                raise Exception(error+' at line '
                                    +str(input_line_index)+' in file '
                                    +input_file_name+' Code:88tnvr')
                            if input[i] != ')':
                                raise Exception('Unexpected character'
                                    +' at line '+str(input_line_index)
                            ...
                            i += 1
                            if i >= input_len:
                                tokens.append(Token(Token.create_state,
                                    position, name,input_line_index,
                                    input_file_name))
                                return tokens
                            else:
                                tokens.append(Token(Token.create_state,
                            ...
                            i += 1
                            if i >= input_len:
                                tokens.append(Token(Token.create_state, None,
                                    name, input_line_index, input_file_name))
                                return tokens
                            else:
                                tokens.append(Token(Token.create_state, None,
                                    name, input_line_index, input_file_name))
                        ...
                        if i >= input_len:
                            raise Exception('Unexpected end of line '
                                +str(input_line_index)+' in file '
                                +input_file_name+' Code:zbk245')
                        if input[i] == '[': i+=1
                        if i >= input_len:
                            raise Exception('Unexpected end of line '
                                +str(input_line_index)+' in file '
                                +input_file_name+' Code:qw3n9i')
                        length, _ = Parser.cut_word(input[i:],separators=']',
                            escape_characters='\\',allowed_chars=None)
                        name = input[i:length+i]
                        i += length+1
                        if i >= input_len:
                            raise Exception('Unexpected end of line '
                                +str(input_line_index)+' in file '
                                +input_file_name+' Code:5w86au')
                        if input[i] != '(':
                            raise Exception('Unexpected character'+' at line '
                                +str(input_line_index)+' in file '
                                +input_file_name+' Code:qgr887')
                        i+=1
                        if i >= input_len:
                            raise Exception('Unexpected end of line '
                                +str(input_line_index)+' in file '
                                +input_file_name+' Code:vo46qi')
                        (length, position,
                            success, error,
                            ) = Parser.parse_coordinates(input[i:],
                                count=6,allow_fraction=False)
                        if not success:
                            (length, position,
                                success, error
                                ) = Parser.parse_coordinates(input[i:],
                        ...
                        if not success:
                            raise Exception(error+' at line '
                                +str(input_line_index)+' in file '
                                +input_file_name+' Code:vzb57n')
                        i += length
                        if not success:
                            raise Exception(error+' at line '
                                +str(input_line_index)+' in file '
                                +input_file_name+' Code:nd64bz')
                        if input[i] != ')':
                            raise Exception('Unexpected character'
                                +' at line '+str(input_line_index)
                                +' in file '+input_file_name+' Code:tzqqb3')
                        i += 1
                        if i >= input_len:
                            raise Exception('Unexpected end of line '
                                +str(input_line_index)+' in file '
                                +input_file_name+' Code:omf5u6')
                        if input[i] != '`':
                            raise Exception('Unexpected character'+' at line '
                                +str(input_line_index)+' in file '
                                +input_file_name+' Code:o9limx')
                        i += 1
                        if i >= input_len:
                            tokens.append(Token(Token.create_position,
                                position, name, input_line_index,
                                input_file_name))
                            return tokens
                        else:
                        ...
                        if i >= input_len:
                            raise Exception('Unexpected end of line '
                                +str(input_line_index)+' in file '
                                +input_file_name+' Code:4yu0rh')
                        if input[i] == '[': i+=1
                        if i >= input_len:
                            raise Exception('Unexpected end of line '
                                +str(input_line_index)+' in file '
                                +input_file_name+' Code:hamfrp')
                        length, _ = Parser.cut_word(input[i:],separators=']',
                            escape_characters='\\',allowed_chars=None)
                        name = input[i:length+i]
                        i += length+1
                        if i >= input_len:
                            raise Exception('Unexpected end of line '
                                +str(input_line_index)+' in file '
                                +input_file_name+' Code:o0rg29')
                        if input[i] != '(':
                            raise Exception('Unexpected character'
                                +' at line '+str(input_line_index)
                                +' in file '+input_file_name+' Code:x2mfvz')
                        i+=1
                        if i >= input_len:
                            raise Exception('Unexpected end of line '
                                +str(input_line_index)+' in file '
                                +input_file_name+' Code:tx5p6h')
                        ...
                        if input[i] != ')':
                            raise Exception('Unexpected character'+' at line '
                                +str(input_line_index)+' in file '
                                +input_file_name+' Code:dnv39i')
                        i += 1
                        if i >= input_len:
                            raise Exception('Unexpected end of line '
                                +str(input_line_index)+' in file '
                                +input_file_name+' Code:xholn8')
                        if input[i] != '`':
                            raise Exception('Unexpected character'
                                +' at line '+str(input_line_index)
                                +' in file '+input_file_name+' Code:14qutf')
                        i += 1
                        if i >= input_len:
                            tokens.append(Token(Token.create_custom, value,
                                name, input_line_index, input_file_name))
                            return tokens
                        else:
                            tokens.append(Token(Token.create_custom, value,
                        ...
                        if i >= input_len:
                            raise Exception('Unexpected end of line '
                                +str(input_line_index)+' in file '
                                +input_file_name+' Code:76lxxq')
                        if input[i] == '[': i+=1
                        if i >= input_len:
                            raise Exception('Unexpected end of line '
                                +str(input_line_index)+' in file '
                                +input_file_name+' Code:tghhi5')
                        length, _ = Parser.cut_word(input[i:], separators=']',
                            escape_characters='\\',allowed_chars=None)
                        name = input[i:length+i]
                        i += length+1
                        if i >= input_len:
                            raise Exception('Unexpected end of line '
                                +str(input_line_index)+' in file '
                                +input_file_name+' Code:zi8n8q')
                        if input[i] != '`':
                            raise Exception('Unexpected character'+' at line '
                                +str(input_line_index)+' in file '
                                +input_file_name+' Code:qtkofv')
                        i += 1
                        if i >= input_len:
                            tokens.append(Token(Token.create_impulse, None,
                                name, input_line_index, input_file_name))
                            return tokens
                        else:
                            tokens.append(Token(Token.create_impulse, None,
                        ...
                        if i >= input_len:
                            raise Exception('Unexpected end of line '
                                +str(input_line_index)+' in file '
                                +input_file_name+' Code:7fbixj')
                        if input[i] == '[': i+=1
                        if i >= input_len:
                            raise Exception('Unexpected end of line '
                                +str(input_line_index)+' in file '
                                +input_file_name+' Code:w4zzkb')
                        length, _ = Parser.cut_word(input[i:],separators=']',
                            escape_characters='\\',allowed_chars=None)
                        name = input[i:length+i]
                        i += length+1
                        if i >= input_len:
                            raise Exception('Unexpected end of line '
                                +str(input_line_index)+' in file '
                                +input_file_name+' Code:9r7t8e')
                        if input[i] != '`':
                            raise Exception('Unexpected character'+' at line '
                                +str(input_line_index)+' in file '
                                +input_file_name+' Code:sfukg3')
                        i += 1
                        if i >= input_len:
                            tokens.append(Token(Token.create_repeat, None,
                                name, input_line_index, input_file_name))
                            return tokens
                        else:
                            tokens.append(Token(Token.create_repeat, None,
                        ...
                        if i >= input_len:
                            raise Exception('Unexpected end of line '
                                +str(input_line_index)+' in file '
                                +input_file_name+' Code:px0vkm')
                        if input[i] == '[': i+=1
                        if i >= input_len:
                            raise Exception('Unexpected end of line '
                                +str(input_line_index)+' in file '
                                +input_file_name+' Code:4fd23c')
                        length, _ = Parser.cut_word(input[i:],separators=']',
                            escape_characters='\\',allowed_chars=None)
                        name = input[i:length+i]
                        i += length+1
                        if i >= input_len:
                            raise Exception('Unexpected end of line '
                                +str(input_line_index)+' in file '
                                +input_file_name+' Code:1rb0e4')
                        if input[i] != '`':
                            raise Exception('Unexpected character'+' at line '
                                +str(input_line_index)+' in file '
                                +input_file_name+' Code:a04wgv')
                        i += 1
                        if i >= input_len:
                            tokens.append(Token(Token.create_dialog, None,
                                name, input_line_index, input_file_name))
                            return tokens
                        else:
                            tokens.append(Token(Token.create_dialog, None,
                    ...
                    if input[i] == 's':#state
                        ...
                        if i >= input_len:
                            raise Exception('Unexpected end of line '
                                +str(input_line_index)+' in file '
                                +input_file_name+' Code:tzc7vm')
                        if input[i] == '[': i+=1
                        if i >= input_len:
                            raise Exception('Unexpected end of line '
                                +str(input_line_index)+' in file '
                                +input_file_name+' Code:zdr77w')
                        length, _ = Parser.cut_word(input[i:], separators=']',
                            escape_characters='\\', allowed_chars=None)
                        name = input[i:length+i]
                        i += length+1
                        if i >= input_len:
                            raise Exception('Unexpected end of line '
                                +str(input_line_index)+' in file '
                                +input_file_name+' Code:qck9d9')
                        if input[i] != '(':
                            raise Exception('Unexpected character'+' at line '
                                +str(input_line_index)+' in file '
                                +input_file_name+' Code:q885lm')
                        i+=1
                        if i >= input_len:
                            raise Exception('Unexpected end of line '
                                +str(input_line_index)+' in file '
                                +input_file_name+' Code:rxslfx')
                        ...
                        if input[i] != ')':
                            raise Exception('Unexpected character'
                                +' at line '+str(input_line_index)+' in file '
                                +input_file_name+' Code:1ohgt1')
                        i += 1
                        if i >= input_len:
                            raise Exception('Unexpected end of line '
                                +str(input_line_index)+' in file '
                                +input_file_name+' Code:jqj1ix')
                        if input[i] != '`':
                            raise Exception('Unexpected character'+' at line '
                                +str(input_line_index)+' in file '
                                +input_file_name+' Code:qd8008')
                        i += 1
                        if i >= input_len:
                            #if value.strip() != '':
                            tokens.append(Token(Token.state, value,name,
                                input_line_index, input_file_name))
                            return tokens
                        else:
                            tokens.append(Token(Token.state, value,name,
                        ...
                        if i >= input_len:
                            raise Exception('Unexpected end of line '
                                +str(input_line_index)+' in file '
                                +input_file_name+' Code:tzx26k')
                        if input[i] == '[': i+=1
                        if i >= input_len:
                            raise Exception('Unexpected end of line '
                                +str(input_line_index)+' in file '
                                +input_file_name+' Code:ll2nko')
                        length, _ = Parser.cut_word(input[i:], separators=']',
                            escape_characters='\\',allowed_chars=None)
                        name = input[i:length+i]
                        i += length+1
                        ...
                        if i >= input_len:
                            raise Exception('Unexpected end of line '
                                +str(input_line_index)+' in file '
                                +input_file_name+' Code:qxfmur')
                        if input[i] != '`':
                            raise Exception('Unexpected character'+' at line '
                                +str(input_line_index)+' in file '
                                +input_file_name+' Code:pu0tq7')
                        i += 1
                        if i >= input_len:
                            tokens.append(Token(Token.position, position_type,
                                name, input_line_index, input_file_name))
                            return tokens
                        else:
                            consumed_input = i
                        ...
                        if i >= input_len:
                            raise Exception('Unexpected end of line '
                                +str(input_line_index)+' in file '
                                +input_file_name+' Code:gcsxnk')
                        if input[i] == '[': i+=1
                        if i >= input_len:
                            raise Exception('Unexpected end of line '
                                +str(input_line_index)+' in file '
                                +input_file_name+' Code:aij5ws')
                        length, _ = Parser.cut_word(input[i:],separators=']',
                            escape_characters='\\',allowed_chars=None)
                        name = input[i:length+i]
                        i += length+1
                        if i >= input_len:
                            raise Exception('Unexpected end of line '
                                +str(input_line_index)+' in file '
                                +input_file_name+' Code:7rmwud')
                        if input[i] != '`':
                            raise Exception('Unexpected character'+' at line '
                                +str(input_line_index)+' in file '
                                +input_file_name+' Code:pdp13e')
                        i += 1
                        if i >= input_len:
                            tokens.append(Token(Token.custom, None, name,
                                input_line_index, input_file_name))
                            return tokens
                        else:
                            consumed_input = i
                        ...
                        if i >= input_len:
                            raise Exception('Unexpected end of line '
                                +str(input_line_index)+' in file '
                                +input_file_name+' Code:we1che')
                        if input[i] == '[': i+=1
                        if i >= input_len:
                            raise Exception('Unexpected end of line '
                                +str(input_line_index)+' in file '
                                +input_file_name+' Code:f2styu')
                        length, _ = Parser.cut_word(input[i:],separators=']',
                            escape_characters='\\',allowed_chars=None)
                        name = input[i:length+i]
                        i += length+1
                        if i >= input_len:
                            raise Exception('Unexpected end of line '
                                +str(input_line_index)+' in file '
                                +input_file_name+' Code:519rgu')
                        if input[i] == '(':
                            i+=1
                            if i >= input_len:
                                raise Exception('Unexpected end of line '
                                    +str(input_line_index)+' in file '
                                    +input_file_name+' Code:lwub02')
                            ...
                            i += 1
                            if i >= input_len:
                                tokens.append(Token(Token.impulse, value,name,
                                    input_line_index, input_file_name))
                                return tokens
                            else:
                                tokens.append(Token(Token.impulse, value,name,
                            ...
                            i += 1
                            if i >= input_len:
                                tokens.append(Token(Token.impulse, 1, name,
                                    input_line_index, input_file_name))
                                return tokens
                            else:
                                tokens.append(Token(Token.impulse, 1, name,
                        ...
                        if i >= input_len:
                            raise Exception('Unexpected end of line '
                                +str(input_line_index)+' in file '
                                +input_file_name+' Code:tmbpwk')
                        if input[i] == '[': i+=1
                        if i >= input_len:
                            raise Exception('Unexpected end of line '
                                +str(input_line_index)+' in file '
                                +input_file_name+' Code:djwcv6')
                        length, _ = Parser.cut_word(input[i:],separators=']',
                            escape_characters='\\',allowed_chars=None)
                        name = input[i:length+i]
                        i += length+1
                        if i >= input_len:
                            raise Exception('Unexpected end of line '
                                +str(input_line_index)+' in file '
                                +input_file_name+' Code:nvpbvu')
                        if input[i] == '(':
                            i+=1
                            if i >= input_len:
                                raise Exception('Unexpected end of line '
                                    +str(input_line_index)+' in file '
                                    +input_file_name+' Code:hoq00m')
                            if input[i] not in '01':
                                raise Exception('Unexpected character'
                                    +' at line '+str(input_line_index)
                                    +' in file '+input_file_name
                            ...
                            i += 1
                            if i >= input_len:
                                tokens.append(Token(Token.repeat, value,name,
                                    input_line_index, input_file_name))
                                return tokens
                            else:
                                tokens.append(Token(Token.repeat, value,name,
                            ...
                            i += 1
                            if i >= input_len:
                                tokens.append(Token(Token.repeat, 1,name,
                                    input_line_index, input_file_name))
                                return tokens
                            else:
                                tokens.append(Token(Token.repeat, 1,name,
                        ...
                        if i >= input_len:
                            raise Exception('Unexpected end of line '
                                +str(input_line_index)+' in file '
                                +input_file_name+' Code:pvcvuy')
                        if input[i] == '[': i+=1
                        if i >= input_len:
                            raise Exception('Unexpected end of line '
                                +str(input_line_index)+' in file '
                                +input_file_name+' Code:8u4tvo')
                        length, _ = Parser.cut_word(input[i:],separators=']',
                            escape_characters='\\',allowed_chars=None)
                        name = input[i:length+i]
                        i += length+1
                        if i >= input_len:
                            raise Exception('Unexpected end of line '
                                +str(input_line_index)+' in file '
                                +input_file_name+' Code:dor3mx')
                        if input[i] == '(':
                            i+=1
                            if i >= input_len:
                                raise Exception('Unexpected end of line '
                                    +str(input_line_index)+' in file '
                                    +input_file_name+' Code:92e5gx')
                            if input[i] not in '01':
                                raise Exception('Unexpected character'
                                    +' at line '+str(input_line_index)
                                    +' in file '+input_file_name
                            ...
                            i += 1
                            if i >= input_len:
                                tokens.append(Token(Token.dialog, value,name,
                                    input_line_index, input_file_name))
                                return tokens
                            else:
                                tokens.append(Token(Token.dialog, value,name,
                            ...
                            i += 1
                            if i >= input_len:
                                tokens.append(Token(Token.dialog, 1,name,
                                    input_line_index, input_file_name))
                                return tokens
                            else:
                                tokens.append(Token(Token.dialog, 1,name,
                ...
                i += 1
                if i >= input_len:
                    tokens.append(Token(Token.comment, input[consumed_input:i]
                        ,None, input_line_index, input_file_name))
                    return tokens
                
                
            if i >= input_len:
                raise Exception('Unexpected end of line '+str(input_line_index)
                    +' in file '+input_file_name+' Code:ng3qnc')
    ...
    def parse_file(self, file_name):
                ...
                with open(full_item_file_name) as f:
                    ...
                    for l in f:
                        ...
                        self.tokens.append(Token(Token.new_line, None, None,
                            line_index, full_item_file_name))
                        line_index += 1    
                        
                        
            ##Get tokens from mcfunctions
            if self.behavior_pack_uuid != None and self.functions_path != None:
        ...
        #tape, value, name
        expect_comment_or_new_line = (Token.create_state,
            Token.create_position, Token.create_custom, Token.create_impulse,
            Token.create_repeat, Token.create_dialog,
            )
        code_tokens = (Token.state, Token.position, Token.custom,
            Token.impulse, Token.repeat, Token.dialog, Token.command,
            Token.custom_name,
            )
            ...
            if t.type == Token.create_state:
                if lastToken != Token.new_line:
                    raise Exception('Unexpected "create_state" token at line'
                        +str(t.line)+' in file '+t.file_name+' Code:bu20ol')
                if t.name in self.states:
                    raise Exception('State '+t.name
                ...
                if t.name in self.positions:
                    raise Exception('Position '+t.name
                        +' is declared more than one time (line '+str(t.line)
                        +' in file '+t.file_name+')'+' Code:stx2kl')
                self.positions[t.name] = t.value
            elif t.type == Token.create_custom:
                if lastToken != Token.new_line:
                    raise Exception('Unexpected "create_custom" token at line'
                        +str(t.line)+' in file '+t.file_name+' Code:js6nq9')
                if t.name in self.custom_values:
                    raise Exception('Custom value '+t.name
                        +' is declared more than one time (line '
                        +str(t.line)+' in file '+t.file_name+')'
                        +' Code:kk1laf')
                ...
                if lastToken != Token.new_line:
                    raise Exception('Unexpected "create_impulse" token at'
                        +'line'+str(t.line)+' in file '+t.file_name
                        +' Code:7ylvl9')
                if t.name in self.impulse_chains:
                    raise Exception('Impulse chain '
                        +t.name+' is declared more than one time (line '
                        +str(t.line)+' in file '+t.file_name+')'
                        +' Code:d2ju0e')
                ...
                if lastToken != Token.new_line:
                    raise Exception('Unexpected "create_repeat" token at line'
                        +str(t.line)+' in file '+t.file_name+' Code:8kazag')
                if t.name in self.repeat_chains:
                    raise Exception('Repeating chain '+t.name
                        +' is declared more than one time (line '
                        +str(t.line)+' in file '+t.file_name+')'
                        +' Code:z8z27x')
                ...
                if is_first:
                    raise Exception('First command in chain cannot be'
                        +'conditional (line '+str(t.line)+' in file '
                        +t.file_name+')'+' Code:x27nue')
                curr_command_line.append(t)
                ...
                if add_to_chain and last_command_group != None:
                    if last_command_group.type == Token.create_impulse:
                        self\
                        .impulse_chains[last_command_group.name]['commands']\
                        .append(curr_command_line)
                    elif last_command_group.type == Token.create_repeat:
                        self\
                        .repeat_chains[last_command_group.name]['commands']\
                        .append(curr_command_line)
            ...
            if t.type == Token.state:
                if lastToken in expect_comment_or_new_line:
                    raise Exception('Unexpected "state" token at line'
                        +str(t.line)+' in file '+t.file_name+' Code:gnazon')
                if t.name not in self.states:
                    raise Exception('Trying to refer to state '+t.name
                        +' but it has on definition (line '+str(t.line)
                        +' in file '+t.file_name+')'+' Code:1lfu6i')
            elif t.type == Token.position:
                if lastToken in expect_comment_or_new_line:
                    raise Exception('Unexpected "position" token at line'
                        +str(t.line)+' in file '+t.file_name+' Code:7p1s9w')
                if t.name not in self.positions:
                    raise Exception('Trying to refer to position '+t.name
                        +' but it has on definition (line '+str(t.line)
                        +' in file '+t.file_name+')'+' Code:3kzunj')
            elif t.type == Token.custom:
                if lastToken in expect_comment_or_new_line:
                    raise Exception('Unexpected "custom" token at line'
                        +str(t.line)+' in file '+t.file_name+' Code:hew5yq')
                if t.name not in self.custom_values:
                    raise Exception('Trying to refer to custom value '
                        +t.name+' but it has on definition (line '
                        +str(t.line)+' in file '+t.file_name+')'
                        +' Code:nbfs33')
            elif t.type == Token.impulse:
                if lastToken in expect_comment_or_new_line:
                    raise Exception('Unexpected "impulse" token at line '
                        +str(t.line)+' in file '+t.file_name+' Code:r8jl1b')
                if t.name not in self.impulse_chains:
                    raise Exception('Trying to refer to impulse chain '+t.name
                        +' but it has on definition (line '+str(t.line)
                        +' in file '+t.file_name+')'+' Code:9k89ng')
            elif t.type == Token.repeat:
                if lastToken in expect_comment_or_new_line:
                    raise Exception('Unexpected "repeat" token at line'
                        +str(t.line)+' in file '+t.file_name+' Code:jn42q3')
                ...
                if lastToken in expect_comment_or_new_line:
                    raise Exception('Unexpected "dialog" token at line'
                        +str(t.line)+' in file '+t.file_name+' Code:cj8fih')
                if t.name not in self.dialogs:
                    raise Exception('Trying to refer to dialog '+t.name
                        +' but it has on definition (line '+str(t.line)
                        +' in file '+t.file_name+')'+' Code:zl5lwq')    
            ...
            #    pass
                ...
                if lastToken in expect_comment_or_new_line:
                    raise Exception('Unexpected "command" token at line'
                        +str(t.line)+' in file '+t.file_name+' Code:yk81ta')
            elif t.type == Token.conditional:
                if lastToken != Token.new_line:
                    raise Exception('Unexpected "conditional" token at line'
                        +str(t.line)+' in file '+t.file_name+' Code:9s38dh')
            elif t.type == Token.new_line:
                pass
            lastToken = t.type
        ...
        for t in self.functions_tokens:
            if t.type == Token.create_state:
                raise Exception('States cannot be defined inside mcfunctions. '
                    +'Line:'+str(t.line)+' File: '+t.file_name+' Code:evdlyg')
            elif t.type == Token.create_position:
                raise Exception('Positions cannot be defined inside'
                    +'mcfunctions. Line:'+str(t.line)+' File: '
                    +t.file_name+' Code:9uqqq4')
            elif t.type == Token.create_custom:
                raise Exception('Custom values cannot be defined inside '
                    +'mcfunctions. Line:'+str(t.line)+' File: '
                    +t.file_name+' Code:i1kiv1')
            elif t.type == Token.create_impulse:#IMPULSE CHAIN
                raise Exception('Impulse chains cannot be defined inside '
                    +'mcfunctions. Line:'+str(t.line)+' File: '+t.file_name
                    +' Code:qfj4lo')
            elif t.type == Token.create_repeat:#REPEATING CHAIN
                ...
                raise Exception('Dialog chains cannot be defined inside '
                    +'mcfunctions. Line:'+str(t.line)+' File: '
                    +t.file_name+' Code:y7jdhd')
            elif t.type in code_tokens:
                if t.type == Token.state:
                    if t.name not in self.states:
                        raise Exception('Trying to refer to state '+t.name
                            +' but it has on definition (line '+str(t.line)
                            +' in file '+t.file_name+')'+' Code:q30qmo')
                elif t.type == Token.position:
                    if t.name not in self.positions:
                        raise Exception('Trying to refer to position '+t.name
                            +' but it has on definition (line '+str(t.line)
                            +' in file '+t.file_name+')'+' Code:1vdck4')
                elif t.type == Token.custom:
                    if t.name not in self.custom_values:
                        raise Exception('Trying to refer to custom value '
                            +t.name+' but it has on definition (line '
                    ...
                    if t.name not in self.dialogs:
                        raise Exception('Trying to refer to dialog '+t.name
                            +' but it has on definition (line '+str(t.line)
                            +' in file '+t.file_name+')'+' Code:dpyerm')
                elif t.type == Token.custom_name:
                    raise Exception('Unexpected "custom_name" token inside '
                        +'mcfunction file. Line:'+str(t.line)+' File: '
                        +t.file_name+' Code:qtm2li')
                curr_command_line.append(t)
            elif t.type == Token.new_dialog_chain:
                raise Exception('Unexpected "new_dialog_chain" token inside '
                    +'mcfunction file. Line:'+str(t.line)+' File: '+t.file_name
                    +' Code:bi9c9t')
            elif t.type == Token.conditional:
    ...
    def command_to_string(self,command_elements):
        ...
        for element in command_elements:
            if element.type == Token.state:
                ...
                if element.value == 'selector':
                    ...
                    if len(positions) == 6:
                        command = command +'x='+str(positions[0]) \
                            +',y='+str(positions[1])+',z='+str(positions[2]) \
                ...
                [str(i) for i in self.impulse_chains[element.name]['position']]
                )+' '+Parser.off_on[element.value]
            elif element.type == Token.repeat:
                command = command + ' '.join(
                [str(i) for i in self.repeat_chains[element.name]['position']]
                )+' '+Parser.off_on[element.value]
...
class Planner:
        ...
        for curr_len in chain_lengths:
            new_x,new_y, new_z,  new_plan, new_direction,success = \
                add_instructions(cursor_x,cursor_y,cursor_z,curr_len,
                    direction)
            if not success:
                cursor_x,cursor_y,cursor_z, direction= curve(cursor_x,
                    cursor_y,cursor_z, direction)
                new_x,new_y, new_z,  new_plan, new_direction,success = \
                    add_instructions(cursor_x,cursor_y,cursor_z,curr_len,
                        direction)
                if not success:
    ...
    def plan_and_build(self,box, level, floor_block, floor_height=255):
        ...
        for k,v in self.impulse_chains.items():
            ...
            if plan == None:
                raise Exception('Not enough space for "'+k
                    +'" impulse chain (increase Z size)'+' Code:uvwsso')
        ...
        for k,v in self.repeat_chains.items():
            ...
            if plan == None:
                raise Exception('Not enough space for "'+k
                    +'" repeating chain (increase Z size)'+' Code:ars5f6')
        ...
        for k,v in self.dialogs.items():
            ...
            if not self.is_space_for_dialog(v,floor_height-1, limz-3):
                raise Exception('Not enough space for "'+k
                    +'" dialog chain (increase Y or Z size)'+' Code:fgs2ir')
        ...
        for k,v in self.parser.states.items():
            if self.parser.states[k] == None:
                self.parser.states[k] = [state_positions[i][0]+box.minx,
                    state_positions[i][1]+box.miny, state_z]
                i += 1
        ...
        for k,v in self.parser.impulse_chains.items():
            if curr_x+2 > limx:
                curr_x = 0
                curr_floor +=1
            self.parser.impulse_chains[k]['position'] = [curr_x+box.minx,
                box.miny+(curr_floor*floor_height)+1, chain_z]
            curr_x += 2
        ...
        for k,v in self.parser.repeat_chains.items():
            if curr_x+2 > limx:
                curr_x = 0
                curr_floor +=1
            self.parser.repeat_chains[k]['position'] = [curr_x+box.minx,
                box.miny+(curr_floor*floor_height)+1, chain_z]
            curr_x += 2
        ...
        for k,v in self.parser.dialogs.items():
            if curr_x+4 > limx:
                curr_x = 0
                curr_floor +=1
            self.parser.dialogs[k]['position'] = [curr_x+box.minx,
                box.miny+(curr_floor*floor_height)+1, chain_z]
            curr_x += 4
                ...
                for move in impulse_chains_projects[k]:
                    ...
                    if move == Planner.zp:
                        ...
                        Builder.place_cmdblock(cx,cy,cz, level, cb_type,
                            Builder.z_up, command, is_conditional, customName)
                        i += 1
                        cz += 1
                        ...
                        (command, is_conditional, is_new_dialog_chain,
                            customName)  = \
                                self.parser.command_to_string(cmd_source[i])
                        Builder.place_cmdblock(cx,cy,cz, level, cb_type,
                            Builder.z_down, command, is_conditional,
                            customName)
                        i += 1
                        cz -= 1
                        ...
                        (command, is_conditional, is_new_dialog_chain,
                            customName) = \
                                self.parser.command_to_string(cmd_source[i])
                        Builder.place_cmdblock(cx,cy,cz, level, cb_type,
                            Builder.y_up, command, is_conditional, customName)
                        i += 1
                        cy += 1
            ...
            for k,v in self.parser.dialogs.items():
                ...
                for command_elements in self.parser.dialogs[k]['commands']:
                    ...
                    if is_new_dialog_chain:
                        if i == 1:
                            #Pierwszy osdstep jest troche krotszy bo nie trzeba
                            # budowac calej struktury
                            cz += 2
                        elif i > 1:
                            cz += 4
                        i+=1
                    ...
                    if i==1:#Pierwszy rzadek
                        if is_first:
                            Builder.place_cmdblock(cx-1,cy,cz, level,
                                Builder.cb_impulse, Builder.y_up,
                                'setblock ~2 ~ ~2 air', False)
                            is_first = False
                        cy += 1
                        Builder.place_cmdblock(cx-1,cy,cz, level,
                            Builder.cb_chain, Builder.y_up, command,
                            is_conditional, customName)
                    else:
                        def build_layer(x,y,z, level):
                            Builder.place_block(x, y, z, alphaMaterials[152,0],
                                level)#redstone_block
                            Builder.place_block(x+1, y, z,
                                alphaMaterials[152,0], level)#redstone_block
                            Builder.place_block(x, y, z+1,
                                alphaMaterials[55,15], level)#redstone_dust
                            Builder.place_hooper(x+1, y, z+1, level,
                                [10],Builder.z_up, True)#hooper disabled south
                            Builder.place_comparator(x,y,z+2, level,
                                Builder.x_up, 0, True)#comparator
                            Builder.place_hooper(x+1, y, z+2, level,
                                [64,64,64,64,54],Builder.z_up, True)#hooper
                        if is_first:
                            build_layer(cx,cy,cz,level)
                            Builder.place_block(cx, cy+1, cz+1, floor_block,
                                level)#floor_block
                            Builder.place_block(cx+1, cy+1, cz+1, floor_block,
                                level)#floor_block
                            Builder.place_block(cx, cy+1, cz+2, floor_block,
                                level)#floor_block
                            Builder.place_block(cx+1, cy+1, cz+2, floor_block,
                                level)#floor_block
                            build_layer(cx,cy+2,cz,level)
                            Builder.place_cmdblock(cx-1,cy,cz+2, level,
                                Builder.cb_impulse, Builder.y_up,
                                'clone ~1 ~2 ~-2 ~2 ~2 ~ ~1 ~ ~-2', False)
                            cy += 1
                            Builder.place_cmdblock(cx-1,cy,cz+2, level,
                                Builder.cb_chain, Builder.y_up,
                                'setblock ~2 ~-1 ~2 air', False)
                            is_first = False
                        cy += 1
                        Builder.place_cmdblock(cx-1,cy,cz+2, level,
                            Builder.cb_chain, Builder.y_up, command,
        ...
        for x in range(box.minx,box.maxx):
            for y in range(box.miny,box.maxy):
                for z in range(box.minz,box.maxz):
                    if (((y-box.miny)%floor_height == 0 and box.minz+2 <= z) or
                    (y==box.miny)):
    ...
    def get_bp_path(self, world_path):
        ...
        if bp_funcpath == None:
            ...
            dev_bp_path = os.path.join(dev_bp_path,
    ...
    def analyse(self):
        #Create list of lengths of conditional chains needed for impulse chains
...
########   FILTER CODE   ###############################################
displayName = "Nusiq's brfunctions - v1.5.1"
...
def perform(level, box, options):
    ...
    myFloorHeight = parser.floor_height if parser.floor_height != None else \
